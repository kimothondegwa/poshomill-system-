...
<div class="stat-cards">
    <div class="stat-card-alert active">
        ...
        <div class="stat-card-alert-value">{{ $alerts->where('status', 'active')->count() }}</div>
    </div>
    <div class="stat-card-alert low-stock">
        <div class="stat-card-alert-label">Low Stock</div>
        <div class="stat-card-alert-value">{{ $alerts->where('alert_type', 'low_stock')->count() }}</div>
    </div>
    <div class="stat-card-alert expiring">
        <div class="stat-card-alert-label">Expiring Soon</div>
        <div class="stat-card-alert-value">{{ $alerts->where('alert_type', 'expiring_soon')->count() }}</div>
    </div>
    <div class="stat-card-alert resolved">
        <div class="stat-card-alert-label">Resolved</div>
        <div class="stat-card-alert-value">{{ $alerts->where('status', 'resolved')->count() }}</div>
    </div>
</div>
...
<div class="table-card">
    <div class="table-card-header">
        ...
        <a href="{{ route('alerts.active') }}" class="table-card-header-link">View Active Only</a>
    </div>
    @if ($alerts->count() > 0)
                ...
                <tbody>
                    @foreach ($alerts as $alert)
                        <tr>
                            ...
                            <td><small class="text-muted">{{ Str::limit($alert->message, 40) }}</small></td>
                            <td>
                                <span class="alert-status @if($alert->status === 'active') status-active @elseif($alert->status === 'acknowledged') status-acknowledged @else status-resolved @endif">
                            ...
                            <td>
                                <a href="{{ route('alerts.show', $alert) }}" class="btn action-btn view" title="View">
...
<div class="modal fade" id="checkAlertsModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content" style="border-radius: 10px; border: none;">
            ...
            <form action="{{ route('alerts.check-levels') }}" method="POST">
                @csrf
